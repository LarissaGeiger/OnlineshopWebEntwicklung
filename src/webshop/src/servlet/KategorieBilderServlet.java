...
public class KategorieBilderServlet extends HttpServlet {
	...
	private void add(Integer b) throws ServletException {
		
		
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con.prepareStatement("UPDATE kategorie SET  bildID = ? WHERE id = (SELECT MAX(iD) FROM kategorie)")) {
